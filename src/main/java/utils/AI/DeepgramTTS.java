...
public class DeepgramTTS {
    ...
    public static void textToSpeech(Book book) {
        ...
        // Cấu trúc văn bản cho việc chuyển đổi thành âm thanh
        ...
        try {
            // Gửi yêu cầu và nhận phản hồi âm thanh
            HttpResponse<InputStream> response = client.send(request, HttpResponse.BodyHandlers.ofInputStream());
            if (response.statusCode() == 200) {
                String audioFilePath = "output.mp3"; // Dùng file duy nhất

                // Nếu file output.mp3 đã tồn tại, xóa nó trước khi tạo lại
                File existingFile = new File(audioFilePath);
                if (existingFile.exists()) {
                    existingFile.delete(); // Xóa file cũ
                }

                // Tạo và ghi âm thanh vào file output.mp3
    ...
    // Phương thức chính để nghe sách từ bookId
